...
function renderTensorDiagram(container, nodes, edges) {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    .force("link", d3.forceLink(edges).id(d => d.id).distance(80).strength(1).iterations(10))
    .force("x", d3.forceX())
    .force("y", d3.forceY())
    .stop();
  ...
  link
    ...
    .attr("x", ({source, target}) => source === target ? source.x + 30 : (source.x + target.x) / 2)
    .attr("y", ({source, target}) => source === target ? source.y - 30 : (source.y + target.y) / 2);
